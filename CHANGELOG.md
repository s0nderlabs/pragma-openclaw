...
### Fixed
- Sync MCP server v0.8.60 — auto-register identity on first fund, fix negative fundedAmount reporting

## [0.2.3] - 2026-03-13

### Fixed
- Sync MCP server v0.8.59 — prevent duplicate identity NFT minting, increase log scan window to 100K blocks

## [0.2.2] - 2026-03-13

### Added
- Sync MCP server v0.8.58 — ERC-8004 identity registration tool, auto-registration on setup, smart account linking

## [0.2.1] - 2026-02-17

### Changed
- Sync MCP server v0.8.57 — remove maxIterations/currentIteration from loop config and create_sub_agent
...
### Fixed
- Sync MCP server v0.8.55 — add explicit `maxValuePerTx` parameter to `create_sub_agent` (was auto-derived as budgetMon/maxCalls, now defaults to full budget)

## [0.1.7] - 2026-02-13

### Fixed
- Sync MCP server v0.8.54 — remove all arbitrary caps on delegation budget and parameter fields (schema-level and handler-level validation)
